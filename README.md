...
- **Audio digest** — `digest_biorxiv.py` generates a spoken MP3 digest of a day's papers (news articles → summaries → abstracts, capped at ~20 min). Can be chained with the fetch cron job. Output optionally copied to a fixed path via `PDF2MD_DIGEST_DEST`.
- **Today's papers** — the calendar and CLI both allow fetching today's date; days fetched mid-day are marked *partial* in the calendar.
...
| `fetch_biorxiv.py` | CLI tool to pre-fetch and cache bioRxiv metadata (schedule with cron). Today's date is now allowed. |
| `digest_biorxiv.py` | CLI tool to generate a daily audio digest MP3 from cached paper metadata |
...
| `tts.py` | Offline text-to-speech via Piper (`en_US-amy-medium`). Voice model auto-downloaded to `storage/voices/` on first use. Includes `build_daily_digest_script` and `daily_digest_mp3` for the digest feature. |
...
| `storage/Biorxiv_papers/{date}/digest.mp3` | Daily audio digest (generated by digest_biorxiv.py or the 🎧 button) |
